...
function getAngleBetweenVectors(x1, x2, y1, y2) {
  const x = Math.sqrt(x1 ** 2 + x2 ** 2);
  const y = Math.sqrt(y1 ** 2 + y2 ** 2);
  const degree = Math.acos((x1 * y1 + x2 * y2) / (x * y));
  return degree;
}
...
function isPrime(n) {
  ...
  for (let i = 1; i < sq; i += 1) {
    for (let j = 1; j <= 5; j += 1) {
      if (n % (6 * i + j) === 0 && 6 * i + j !== n) return false;
    }
  }
...
function getSumOfDigits(num) {
  return `${num}`.split('').reduce((acc, item) => acc + item * 1, 0);
}
...
function getSumOfNumbers(x1, x2, x3) {
  return Math.round((x1 + x2 + x3) * 1000) / 1000;
}
